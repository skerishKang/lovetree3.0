...
| `CI_PASSED` | 필수 CI가 exact HEAD에서 성공함 | 필수 check가 성공으로 종료 | exact HEAD, run/job/step, conclusion, log result | CI 시스템, 웹 개발자 보고 | `LOCAL_VALIDATION_REQUIRED`, `CTO_REVIEW` |
...
| `LOCAL_PASSED` | exact HEAD가 소스 무수정 실환경 검증을 통과함 | 필수 local checks 성공 | remote/local SHA 일치, clean status, 환경·명령·UI·통합 증거 | 로컬 검증자 | `CTO_REVIEW` |
...
| `MERGED` | 사용자가 승인한 exact HEAD가 default branch에 병합됨 | 사용자 명시 승인 및 merge 성공 | 승인 기록, PR head, merge SHA | 사용자 승인 후 merge 담당자 | `PRODUCTION_VERIFICATION_REQUIRED` |
| `PRODUCTION_VERIFICATION_REQUIRED` | 병합 후 production 확인이 필요함 | merge 완료 또는 production 배포 시작 | merge SHA, deployment ID·commit·URL | 웹 CTO, 배포 담당자 | `PRODUCTION_FAILED`, `PRODUCTION_VERIFIED`, `BLOCKED` |
| `PRODUCTION_FAILED` | production smoke 또는 revision 일치 검증 실패 | 배포 오류, commit 불일치, runtime 결함 | deployment metadata, 오류, screenshot/log, rollback reference | 웹 CTO, 배포 담당자 | `PRODUCTION_VERIFICATION_REQUIRED`, `BLOCKED` |
| `PRODUCTION_VERIFIED` | 승인한 revision의 production 배포와 smoke가 검증됨 | deployment commit 일치 및 필수 smoke 성공 | merge SHA, deployment SHA·ID·URL, health/UI/API 결과 | 웹 CTO | 후속 작업의 `PLANNING` |
...
- `MERGED`는 `PRODUCTION_VERIFIED`가 아닙니다.
...
- 로컬 검증 중 제품 소스가 수정되면 해당 run은 `LOCAL_PASSED` 증거가 될 수 없습니다.
