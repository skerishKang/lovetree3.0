# LoveTree 3.0 AI 개발 운영정책 v2.0

## 1. 목적

이 정책은 LoveTree 3.0 개발에서 설계·범위 결정, GitHub 구현, 실환경 검증, 최종 승인 권한을 분리한다. 목표는 구현 속도를 높이면서도 자기검증 편향, revision 불일치, 증거 없는 완료 선언, 사용자 승인 없는 merge를 방지하는 것이다.
...
- 하위 `AGENTS.md`가 존재하면 해당 하위 범위에서 더 구체적인 규칙이 우선하지만 이 정책의 역할·승인 분리를 약화할 수 없다.
...
6. **실패를 숨기지 않음**: 실패, pending, 증거 부재, 외부 의존성 미확인은 그대로 기록한다.
...
10. **승인 분리**: `READY`는 merge 명령이 아니며, merge와 deployment는 사용자의 명시적 승인 뒤에만 수행한다.
...
- 최종 merge 및 deployment를 승인하거나 거부한다.
- 제품 의미가 바뀌는 예외를 승인한다.
...
- 실제 diff, commit chain, CI, local evidence, deployment evidence를 독립 검토한다.
- 최종 상태를 `READY`, `CONDITIONALLY_READY`, `NOT_READY`, `BLOCKED` 중 하나로 판정한다.
- 원칙적으로 구현 코드를 수정하지 않는다. 긴급 예외는 제9절 절차를 따라 역할 충돌과 재검증 필요성을 기록한다.
...
- `READY` 또는 merge 가능 여부를 최종 판정하지 않는다.
...
- Firebase, LoveBud, Cloudflare, 환경변수, secret, 외부 계정 등 CI가 대체할 수 없는 연동을 검증한다.
...
- **Owner**: 웹 개발자
- **Input**: 승인된 작업계약
- **Required evidence**: branch base, commit chain, changed files, diff, local commands와 exit code, Draft PR, Actions 및 Cloudflare metadata
...
- **Exit criteria**: 소스 수정 없이 필수 실환경 검증이 통과함
...
- **Required evidence**: scope matrix, acceptance criteria matrix, code/test/security/regression/deployment review
...
- **Required evidence**: 사용자 명시 승인, merge 직전 exact PR HEAD, merge 결과 SHA
- **Exit criteria**: 승인된 exact HEAD가 허용된 방식으로 merge됨
- **Failure transition**: 승인이 없으면 `READY` 또는 `CONDITIONALLY_READY` 유지; head 변경 시 `CTO_REVIEW`로 복귀

### Gate 6 — Production Verification / `PRODUCTION_VERIFICATION_REQUIRED` → `PRODUCTION_VERIFIED`

- **Owner**: 지정 배포 담당자와 웹 CTO
- **Input**: merge SHA, production deployment
- **Required evidence**: production deployment SHA·ID·URL, health/smoke 결과, 핵심 UI·API 확인, rollback reference
- **Exit criteria**: production이 merge 대상 revision을 실행하며 필수 smoke가 성공함
- **Failure transition**: `PRODUCTION_FAILED`; 영향이 지속되면 rollback 또는 새 작업계약으로 전환
...
수신자는 exact base 일치와 manifest를 확인한 뒤 적용한다. 경로 조정이나 충돌 해결로 내용이 바뀌면 새 diff와 새 HEAD를 기준으로 검증한다.

### Mode C — Local Environment Validation

Windows, WSL, 실제 browser, Firebase, Cloudflare, LoveBud API, secret, 외부 계정처럼 GitHub CI만으로 검증할 수 없는 항목을 담당한다.
...
- pending, skipped, cancelled를 success로 표현
...
- 웹 CTO가 긴급 구현을 수행한 경우 역할 충돌을 기록하고, 별도 웹 개발자 또는 독립 검토자가 diff를 재검토하며 로컬 검증을 생략하지 않는다.
...
## 11. Merge 이후 Production 검증

- merge 결과 SHA를 기록하고 production deployment가 해당 SHA 또는 승인된 산출물을 사용했는지 확인한다.
- deployment ID, deployment commit, production URL, 시작·완료 상태를 기록한다.
...
- 실패 시 `PRODUCTION_FAILED`로 전환하고 영향, rollback 가능성, rollback reference, 후속 Issue를 기록한다.
...
4. 역할 분리, exact revision, 사용자 merge 승인 원칙을 약화하는 변경은 사용자 명시 승인이 필요하다.
...
- 배포 URL만 확인하지 않고 deployment commit이 검증한 commit과 일치하는지 확인한다.
