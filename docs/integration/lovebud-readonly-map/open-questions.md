...
| Q2 | What is the LoveBud public API base URL for LoveTree proxy upstream? (NOT MODAL_BASE_URL — LoveTree calls LoveBud public API origin, not Modal directly) | Proxy target configuration | Issue 2 |
...
- Q33: zero-network test migration strategy

Issue 1 is typed client + error normalization only. Firebase project and deployment target are NOT needed.

### Must Resolve Before Issue 2
- Q2: LoveBud public API upstream base URL (NOT MODAL_BASE_URL — LoveTree calls LoveBud public API origin)
- Q34: LoveTree deployment target confirmation (Cloudflare Pages?)
- Proxy server environment/configuration approach

### Must Resolve Before Issue 3
- Q20: Firebase project selection
- Q21: LoveBud account sharing
- Q37: auth token persistence strategy (PRODUCT/SECURITY DECISION)

### Must Resolve Before Issue 5
- Q7: BrowseSnapshot social count availability
- Q11: community list maximum limit

Q1 (production Modal CORS) is NOT a blocker for Issue 5 — same-origin LoveTree proxy calls LoveBud public API, so browser-to-Modal CORS is irrelevant for screen implementation. Q1 is reclassified as deployment verification / operational hardening.
...
### Deployment Verification / Operational Hardening
- Q1: production Modal CORS value (not a screen implementation blocker)
- Q3: production DB identity
- Q4: production schema state
- Q5: staging environment
- Q6: Modal cold-start latency
