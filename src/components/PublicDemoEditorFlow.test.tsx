...
describe("public demo editor flow", () => {
  ...
  it("creates a root, child, and grandchild, then updates a parent and connector", async () => {
    const user = userEvent.setup();
    ...
    await waitFor(() => {
      const saved = JSON.parse(localStorage.getItem(PUBLIC_DEMO_STORAGE_KEY) ?? "null") as PublicDemoDraft;
  ...
  it("enforces the 12-node limit", () => {
    ...
    const nodes = [root, ...Array.from({ length: 11 }, (_, index) => makeNode(`child-${index}`, "root", `기억 ${index + 1}`))];
  ...
  it("rejects invalid YouTube URLs and never commits them", async () => {
    seedDraft([], null);
    const user = userEvent.setup();
  ...
  it("offers subtree deletion for a non-root node with children", async () => {
    ...
    const saved = JSON.parse(localStorage.getItem(PUBLIC_DEMO_STORAGE_KEY) ?? "null") as PublicDemoDraft;
    expect(saved.nodes.map((node) => node.id)).toEqual(["root"]);
  });
  ...
  it("restores a valid draft after remount and reset removes only the exact key with Escape focus return", async () => {
    const user = userEvent.setup();
  ...
  it("preview reflects edited draft and public demo makes zero fetch/XHR calls", () => {
    ...
    expect(screen.getByText("브라우저 draft 설명")).toBeInTheDocument();
