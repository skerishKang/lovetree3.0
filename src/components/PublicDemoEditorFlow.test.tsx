...
function readDraft(): PublicDemoDraft | null {
  const raw = localStorage.getItem(PUBLIC_DEMO_STORAGE_KEY);
  return raw ? (JSON.parse(raw) as PublicDemoDraft) : null;
}

async function completeMemoryForm(title: string, parentTitle?: string) {
...
async function expectStoredNodeCount(count: number) {
  await waitFor(() => {
    expect(readDraft()?.nodes ?? []).toHaveLength(count);
  });
}
...
describe("PublicDemoEditorFlow — typed final contract", () => {
  ...
  it("creates root, child, and grandchild with YouTube lifecycle and zero backend writes", async () => {
    ...
    await completeMemoryForm("루트 기억");
    expect(
      screen.getByRole("img", { name: "루트 기억 YouTube 썸네일" }),
    ).toBeInTheDocument();
    ...
    expect(screen.getByTestId("youtube-player").getAttribute("src")).not.toContain(
      "autoplay",
    );
    ...
    await completeMemoryForm("자식 기억", "루트 기억");
    await user.click(screen.getByRole("button", { name: "기억 저장" }));
    await waitFor(() =>
      expect(screen.getByTestId("node-count")).toHaveTextContent("2 / 12"),
    );

    await user.click(screen.getByRole("button", { name: "기억 추가" }));
    await completeMemoryForm("손자 기억", "자식 기억");
    await user.click(screen.getByRole("button", { name: "기억 저장" }));
    await waitFor(() =>
      expect(screen.getByTestId("node-count")).toHaveTextContent("3 / 12"),
    );
    expect(screen.getAllByTestId("connector")).toHaveLength(2);

    await expectStoredNodeCount(3);
  ...
  it("excludes self and descendants, then reassigns a parent and connector", async () => {
    seedDraft(
      [
        makeNode("root", null, "루트"),
    ...
    const childParentSelect = screen.getByLabelText("부모 기억");
    expect(
      within(childParentSelect).queryByRole("option", { name: "자식" }),
    ).not.toBeInTheDocument();
    expect(
      within(childParentSelect).queryByRole("option", { name: "손자" }),
    ).not.toBeInTheDocument();
    await user.click(screen.getByRole("link", { name: "취소" }));

    await user.click(screen.getByRole("button", { name: /손자/ }));
    await user.click(screen.getByRole("button", { name: "기억 편집" }));
    await user.selectOptions(screen.getByLabelText("부모 기억"), "루트");
    await user.click(screen.getByRole("button", { name: "변경 저장" }));

    await waitFor(() =>
      expect(readDraft()?.nodes.find((node) => node.id === "grand")?.parentId).toBe(
        "root",
      ),
    );
    expect(
      screen.getAllByTestId("connector").some(
        (connector) =>
          connector.getAttribute("data-from-id") === "root" &&
          connector.getAttribute("data-to-id") === "grand",
      ),
    ).toBe(true);
  });

  it("rejects invalid YouTube hosts and malformed URLs without storing them", async () => {
    seedDraft([], null);
    const user = userEvent.setup();
    ...
    for (const invalidUrl of [
      ...
      expect(screen.getByRole("alert")).toHaveTextContent(
        "지원되는 공개 YouTube 영상 URL",
      );
      expect(
        screen.queryByRole("img", { name: /YouTube 썸네일/ }),
      ).not.toBeInTheDocument();
      await user.click(screen.getByRole("button", { name: "기억 저장" }));
  ...
  it("blocks root descendant deletion and deletes a leaf", async () => {
    seedDraft(
      [makeNode("root", null, "루트"), makeNode("leaf", "root", "leaf")],
      "root",
    );
    const user = userEvent.setup();
    renderAppAt("/tree/new-demo/edit");
  ...
  it("supports subtree deletion and child reattachment deletion", async () => {
    ...
    await user.click(
      within(dialog).getByRole("button", { name: "하위 기억 전체 삭제" }),
    );
    await expectStoredNodeCount(1);
    ...
    await user.click(
      within(dialog).getByRole("button", {
        name: "직접 자식 재연결 후 대상만 삭제",
      }),
    );
    await waitFor(() => {
      const saved = readDraft();
      expect(saved?.nodes.some((node) => node.id === "child")).toBe(false);
      expect(saved?.nodes.find((node) => node.id === "grand")?.parentId).toBe(
        "root",
      );
    });
  });

  it("deletes the only root and returns to the empty start", async () => {
  ...
  it("restores after reload and recovers corrupt storage by removing only the exact key", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: "테스트 러브트리" }),
    ).toBeInTheDocument();
    cleanup();
  ...
  it("keeps preview title, description, graph, selection, and YouTube consistent", async () => {
    seedDraft(
      [
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: "편집된 미리보기" }),
    ).toBeInTheDocument();
